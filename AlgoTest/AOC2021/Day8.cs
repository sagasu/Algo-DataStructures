...
namespace AlgoTest.AOC2021
{
    ...
    public class Day8
    {
        ...
        public void Test()
        {
            ...
            {
                var data = line.Split('|');
                var digits = data[0].Split(" ").Where(x => !string.IsNullOrWhiteSpace(x)).ToArray();
                var num = data[1].Split(" ").Where(x => !string.IsNullOrWhiteSpace(x)).ToArray();

                var lookup = new Dictionary<string,int>(); 
                for (var i = 0; i < digits.Length; i++)
                {
                    var str = digits[i].ToCharArray();
                    Array.Sort(str);
                    lookup.Add(string.Join("", str), i);
                }

                var sizes = new int[] {2, 3, 4, 7};
                count += num.Count(t => sizes.Contains(t.Length));
            }
