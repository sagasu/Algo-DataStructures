...
public class Day1
{
    ...
    public void Test2()
    {
        ...
        foreach (var line in lines)
        {
            ...
            {
                if (int.TryParse(trimLine[i].ToString(), out var number))
                {
                    numbers.Add(number);
                }
                else
                {
                    var num = GetNumber(i, trimLine);
                    if (num > 0)
                    {
                        numbers.Add(num);
    ...
    private int GetNumber(int lineIndex, string line)
    {
        for (var i = 0;i<stringNumbers.Length ;i++)
        {
            var index = lineIndex;
            var ind = 0;
            while (index < line.Length && ind < stringNumbers[i].Length)
            {
                if (line[index] == stringNumbers[i][ind])
                {
                    index += 1;
                    ind += 1;

                    if (ind == stringNumbers[i].Length - 1) return i + 1;
                }
                else
                    break;
            }
    ...
    private string[] stringNumbers = new[]
    {
