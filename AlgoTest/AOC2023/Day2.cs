...
public class Day2
{
    ...
    public void Test1()
    {
        var games = data1.Split("\n",StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries);
        var ballLimit = new Dictionary<string, int>()
        {
            {"red", 12},
            {"green", 13},
            {"blue", 14},
        ...
        {
            var gameData = games[i].Split(':')[1];
            var sets = gameData.Split(new char[]{',', ';'}, StringSplitOptions.TrimEntries);
            var isPossible = true;
            for (var j = 0; j < sets.Length; j++)
            {
                var setData = sets[j].Split(' ');
                var ballNr = int.Parse(setData[0]);
                var colour = setData[1];
                if (ballLimit[colour] < ballNr)
                {
                    isPossible = false;
                    break;
                }
    ...
    public void Test2()
    {
        ...
        for (var i = 0; i < games.Length; i++)
        {
            ...
            var maxColour = new Dictionary<string, int>();
            for (var j = 0; j < sets.Length; j++)
            {
                var setData = sets[j].Split(' ');
                var ballNr = int.Parse(setData[0]);
                var colour = setData[1];
                if (!maxColour.TryAdd(colour, ballNr))
                    maxColour[colour] = Math.Max(maxColour[colour], ballNr);
            }

            var product = maxColour.Values.Aggregate(1, (current, maxColourValue) => current * maxColourValue);
