...
using AlgoTest.LeetCode.Time_Based_Key_Value_Store;
 ...
 private int AllFallDown(Dictionary<int, Block> blocks, int ignore = -1)
    ...
    public void Test2()
    {
            foreach (var input in data.Split('\n', StringSplitOptions.TrimEntries))
            {
                ...
                if (match.Success)
                {
                    Brick nb = new Brick(int.Parse(match.Groups["x1"].Value), int.Parse(match.Groups["y1"].Value),
                        int.Parse(match.Groups["z1"].Value),
                        int.Parse(match.Groups["x2"].Value), int.Parse(match.Groups["y2"].Value),
            ...
            {
                Console.Write(
                    $"Brick {b.x1},{b.y1},{b.z1} ~ {b.x2},{b.y2},{b.z2} => supports {b.supports.Count()} and is supported by {b.supportedby.Count()}. Disintegrateble? ");
...
class Brick
{
    ...
    public int ChainReaction(SortedList<int, Brick> bsl, SortedList<int, Brick> fb)
    {
        ...
        Day22.hasfallen.Add(this.sortkey);
        ...
        while (fb.Count() > 0)
        {
            ...
            foreach (int s in bsl[key].supports)
            {
                ...
                foreach (int rs in bsl[s].supportedby)
                    if (!Day22.hasfallen.Contains(rs))
                        remainingsupport++;

                if (remainingsupport == 0) // The brick supports a brick with no other support
                {
                    if (Day22.hasfallen.Add(bsl[s].sortkey)) // Add if not already fallen
                        fb.Add(bsl[s].SortKeyTop(), new Brick(bsl[s]));
                }
            }
        }

        return Day22.hasfallen.Count() - 1;
    }
    ...
    public int ChainReaction2(SortedList<int, Brick> bsl, SortedList<int, Brick> fb)
    {
        ...
        do
        {
            ...
            foreach (Brick b in bsl.Values)
            {
                if (Day22.hasfallen.Contains(b.sortkey)) // No need to check fallen bricks
                    continue;
                ...
                foreach (int s in b.supportedby)
                    if (!Day22.hasfallen.Contains(s))
                        remainingsupport++;

                if (remainingsupport == 0) // The brick supports a brick with no other support
                {
                    Day22.hasfallen.Add(b.sortkey); // Add if not already fallen
                    fell++;
                }
            }
