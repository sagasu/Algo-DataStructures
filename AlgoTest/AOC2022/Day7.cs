...
namespace AlgoTest.AOC2022
{
    ...
    public class Day7
    {
        ...
        private int TraverseTree(TreeNode tree)
        {
        ...
        string BuildPath(string a, string b) => $"{a}/{b}";

        private int BuildTree(string[] data, int i, TreeNode tree)
        {
            if(data.Length <= i) return i;
            ...
            if (row.StartsWith("$"))
            {
                ...
                {
                    var nodeName = row.Replace("$ cd ", "");
                    var treeNode = tree.nodes[BuildPath(tree.path,nodeName)];
                    i = BuildTree(data,i+1,treeNode);
                    tree.val += treeNode.val;
                }
                //ignore ls
            }
            else if (row.StartsWith("dir"))
            {
                var path = row.Replace("dir ", "");
                path = BuildPath(tree.path, path);
                tree.nodes.Add(path, new TreeNode(path));
            }
            else
            {
                var fileSize = int.Parse(row.Split(' ')[0]);
                tree.val += fileSize;
            }
            return BuildTree(data, i+1, tree);
        }
