...
public class FindCheapestPriceSolution
{
    ...
    public int FindCheapestPrice(int n, int[][] flights, int src, int dst, int k)
    {
        ...
        var flightsMap = new Dictionary<int, List<GraphListNode>>();
        var price = new int[n];
    ...
    private void BFS(Dictionary<int, List<GraphListNode>> flightsMap, int src, int dst, int k, int[] price)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            {
                var flights = flightsMap.ContainsKey(queueData.currentNode)
                    ? flightsMap[queueData.currentNode]
                    : new List<GraphListNode>();

                foreach (GraphListNode flight in flights)
                {
                    if (queueData.hopTillNode <= k && queueData.priceTillNow + flight.weight < minPrice
                                                   && (price[flight.destination] == 0 || price[flight.destination] >
                                                       queueData.priceTillNow + flight.weight))
                    {
